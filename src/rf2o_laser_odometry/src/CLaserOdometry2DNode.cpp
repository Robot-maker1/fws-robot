...
void CLaserOdometry2DNode::publish()
{
  ...
  odom.header.stamp = get_clock()->now(); //get_clock()->now(); //initially rf2o_ref.last_odom_time;
  ...
  if (publish_tf)
  {
    ...
    odom_trans.header.stamp = get_clock()->now(); //get_clock()->now(); //initially rf2o_ref.last_odom_time;
