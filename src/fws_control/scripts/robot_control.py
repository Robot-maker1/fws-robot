...
class Commander(Node):
    ...
    def __init__(self):
        ...
        # For AX-18 max Speed is 97[rpm] at 12V
...
class Joy_subscriber(Node):
    ...
    def listener_callback(self, data):
        global vel_msg
        vel_msg.linear.x = data.axes[1]*self.MAX_RPM*math.pi*0.026/30 # wheel radius:0.026m
        vel_msg.linear.y = data.axes[0]*self.MAX_RPM*math.pi*0.026/30
        # the diameter of the radisu (diagonal distance between two wheels) = 0.166 + 0.03*2
        vel_msg.angular.z = -data.axes[3]*2*math.pi*(self.MAX_RPM/60)*2*math.pi*0.026/(0.226*math.pi)
        #print(vel_msg)
